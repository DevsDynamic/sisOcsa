...
<body><main class="wrap">
 <section class="head">
  ...
  <div class="summary"><div class="metric"><strong>{{ $summary['total'] }}</strong><span>Unidades con actividad</span></div><div class="metric"><strong style="color:#168443">{{ $summary['success'] }}</strong><span>Último envío aceptado</span></div><div class="metric"><strong style="color:#c6283e">{{ $summary['error'] }}</strong><span>Último envío rechazado</span></div></div>
 </section>
 <section class="card"><h2>Último resultado por unidad</h2><p class="muted">Ordenado desde el envío más reciente. No expone coordenadas, tokens ni nombres de clientes.</p><div class="legend">“Rechazado” significa que Osinergmin respondió, pero no aceptó la trama. El motivo mostrado es el texto devuelto por su API.</div>
  <div class="table"><table><thead><tr><th>Fecha de envío</th><th>Placa</th><th>Resultado</th><th>Motivo / respuesta</th><th class="optional">Sugerencia</th></tr></thead><tbody>
  @forelse($units as $unit) @php($accepted = $unit->response_status === 'SUCCESS')
   <tr><td class="date">{{ $unit->created_at?->format('d/m/Y H:i:s') }}</td><td class="plate">{{ $unit->plate ?: $unit->uuid }}</td><td><span class="badge {{ $accepted ? 'ok':'error' }}">{{ $accepted ? 'ACEPTADO':'RECHAZADO' }}</span></td><td class="reason">{{ $unit->response_message ?: ($accepted ? 'Trama recibida correctamente.' : 'Osinergmin no devolvió un motivo descriptivo.') }}</td><td class="optional">{{ $unit->response_suggestion ?: '-' }}</td></tr>
